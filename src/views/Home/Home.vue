<template>
  <div class="home">
    <Header :name="templateInfo.name" :size="size"
            @roofPlacement="roofPlacement" @bottomSetting="bottomSetting"
            @locking="locking"
            @plus="plus"
            @reduce="reduce"
            @adaption="adaption"
    ...
    <main class="warp">
      ...
      <section class="scrollBox">
        <vuescroll ref="vs" :ops="ops" @handle-scroll="handleScroll">
          <div id="zoom" :style="{width: zoomW+'px',height: zoomH+'px'}">
            <div class="rulers" :style="{width: zoomW+'px',height: zoomH+'px'}">
              <div class="origin" :style="{top:origin.top+'px',left:origin.left+'px'}"></div>

              <nav @mousemove="navH($event,true)" @mouseout="navH($event,false)" @click="clickNavH" class="nav-h"
                   :style="{top:origin.top+'px',backgroundPositionX:ruler.h%50+'px'}" v-if="rulersSize">

                  <span class="h-point" v-for="(item,index) in rulersSize.hRulers " :key="-index+'hpx'"
                        :style="{left:ruler.h-index*50+'px'}"
                  >{{index*-50}} </span>

                <span class="h-point" v-for="(item,index) in rulersSize.loseHRulers " :key="index+'h'"
                      :style="{left:ruler.h+index*50+'px'}" :id="index===0?'pointh':null"
                >{{index*50}}</span>
              </nav>

              <nav @mousemove="navV($event,true)" @mouseout="navV($event,false)" @click="clickNavV" class="nav-v"
                   :style="{left:origin.left+'px',backgroundPositionY:ruler.v%50+'px'}" v-if="rulersSize">

                <span class="v-point" v-for="(item,index) in rulersSize.vRulers" :key="index+'vpx'"
                      :style="{top:ruler.v-index*50-18+'px'}"
                >{{index*-50}}</span>

                <span class="v-point" v-for="(item,index) in rulersSize.loseVRulers" :key="index+'v'"
                      :style="{top:ruler.v+index*50-18+'px'}" :id="index===0?'pointv':null"
                >{{index*50}}</span>
              </nav>

              <div class="h-move-line" :style="isShowNavH"></div>
              <div class="v-move-line" :style="isShowNavV"></div>

              <div class="h-line" v-for="(item,index) in hLine" :style="item.style" :ref="item.id"
                   :key="item.id" @mousedown="clickLine($event,index,'x')"
                   @mousemove="moveLine(index,'x')" @mouseout="outLine(index,'x')"
                   :title="parseInt(item.style.left)-ruler.h-14+'px'">
                <i v-if="item.close" class="el-icon-error h-line-close" title="删除"
                   @mousemove="moveLine(index,'x')" @click="deleteLine(index,'x')"
                   :style="{top:origin.top+36+'px'}"></i>
              </div>

              <div class="v-line" v-for="(item,index) in vLine" :style="item.style" :ref="item.id"
                   :key="item.id" @mousedown="clickLine($event,index,'y')"
                   @mousemove="moveLine(index,'y')" @mouseout="outLine(index,'y')"
                   :title="parseInt(item.style.top)-ruler.v-14+'px'">
                <i v-if="item.close" class="el-icon-error h-line-close" title="删除"
                   @mousemove="moveLine(index,'y')" @click="deleteLine(index,'y')"
                   :style="{left:origin.left+36+'px'}"></i>
              </div>
            </div>
            <div id="container">
              <div class="canvas-box" :style="matrixStyle">
                <div ref="canvas" :style="canvasStyle" id="canvas" @drop="drop" @dragover="dragover"
                     @click.self="closeIsActive">
                  <vue-drag-resize :isActive="item.isActive" :ref="item.ref" :x="item.x" :y="item.y" :w="item.w"
                                   :h="item.h"
                                   :isDraggable="item.isDraggable" :isResizable="item.isResizable "
                                   @clicked="clicked($event,index)" @resizing="resizing($event,index)"
                                   @dragging="dragging($event,index)"
                                   :minh="minhMinw" :minw="minhMinw"
                                   :parentLimitation="true" v-for="(item,index) in canvasData" :key="index"
                                   :style="item.style">
                    <input v-if="item.text" type="text" v-model="item.message" class="text-info"/>
                    <table v-if="item.isTable" class="table">
                      <tr v-for="table in item.table">
                        <td v-for="(val,i) in table.value" :key="i">
                          <input type="text" v-model.trim="val.msg" @mousedown.stop="handleTdInput"
                                 style="width: 100%;height: 100%;border: none;text-align: center">
                        </td>
                      </tr>
                    </table>
                    <img v-if="item.isPhoto" :src="item.img" width="100%" height="100%">
                  </vue-drag-resize>
                </div>
              </div>
            </div>
      ...
      <ReightSet :message="message" :templateInfo="templateInfo"/>
      <el-dialog title="请输入生成表格的行和列" :before-close="closeTable"
                 :visible.sync="isRenderTable" top="25%"
                 width="400px">
        <div>
          行：
          <el-input-number size="mini" v-model.trim="tableInfo.rows" :min="1"/>
          列：
          <el-input-number size="mini" v-model.trim="tableInfo.lines" :min="1"/>
        </div>
        <div slot="footer" class="dialog-footer">
          <el-button @click="closeTable" size="mini">取 消</el-button>
          <el-button type="primary" @click="renderTable" size="mini">确 定</el-button>
        </div>
      </el-dialog>
    </main>
  ...
  export default {
    ...
    methods: {
      ...
      renderElement(event, type) {
        ...
        //2.1.根据type生成不同类型的数据
        const componentsType = this.renderTypeComponents(type)
        //2.2 合拼两组数据，data在前
